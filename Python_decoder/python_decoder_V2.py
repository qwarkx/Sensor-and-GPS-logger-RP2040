...
def export_sensor_data_mat(path, data, size, compression=1):
    ...
    try:
        if compression:
            sp.savemat(path, {'data': export_data}, appendmat=False, do_compression=True)
            return 1
        else:
            sp.savemat(path, {'data': export_data}, appendmat=False)
            return 1
...
def main():
    ...
    with open(file, mode='rb') as log_data:
        ...
        if exporter:
            ...
            export_sensor_data_mat(file_export_name, full_data, 0)
        ...
        timestamp = np.array(full_data[0:, 0], dtype='d') / 1000000.0
        # timestamp = np.array(full_data[0:, 2], dtype='d') / 1000000.0
        packet_counter = np.array(full_data[0:, 2], dtype='u4')
        sample_counter = full_data[0:, 1]

        time = np.diff(timestamp)

        longitude = np.array(full_data[0:, -2])
        latitude = np.array(full_data[0:, -1])
        ...
        see_hight = np.array(full_data[0:, -4], dtype='d')
